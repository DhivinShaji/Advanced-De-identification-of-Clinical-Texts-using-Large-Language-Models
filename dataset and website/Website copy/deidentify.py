...
from transformers import BertTokenizer, AutoModelForTokenClassification, BertModel, BertTokenizerFast
import torch
...
class NER_BioBERT:
    ...
    model = AutoModelForTokenClassification.from_pretrained('dmis-lab/biobert-base-cased-v1.1', num_labels=len(tag2idx))
    max_length = 512  # Maximum sequence length for BERT

    # Initialize the NER_ClinicalBERT class
    def __init__(self, model_name):
        self.model_name = model_name
        if os.path.exists("../../Models/NER_BioBERT.pt"):
            state_dict = torch.load("../../Models/NER_BioBERT.pt", map_location=self.device)
            self.model.load_state_dict(state_dict)  # Load the model state if available
        else:
            print("Using pre-trained NER_BioBERT model")
...
def deidentify_text(xml_file_path, action):
    ...
    if 'biobert' in model_name:
        ...
        ner_model = NER_BioBERT(model_name)
...
def calculate_k_anonymity(entities):
    # Group entities by quasi-identifiers (QIDs)
    # In this case, we consider both the entity and its surrounding context
    groups = {}
    
    for entity, context, entity_type in entities:
        context = (entity_type, entity)  # Use both entity type and value as quasi-identifier
        if context in groups:
            groups[context].append((context))  # Store positions of similar entities
        else:
            groups[context] = [(context)]

    # Calculate k-anonymity: find the smallest group size
    group_sizes = [len(group) for group in groups.values()]
    
    # If all entities are in unique groups, k-anonymity is 1
    if not group_sizes:
        return 1  # No groups found
    
    k_anonymity = min(group_sizes)
    return k_anonymity

# Function to calculate l-diversity of the entities
def calculate_l_diversity(entities):
    """Calculate l-diversity by checking the number of distinct sensitive values in each group."""
    groups = defaultdict(set)
    
    # Group entities based on their quasi-identifiers (e.g., name + location)
    for entity, context, entity_type in entities:
        key = (entity_type,)  # Use entity type as the key (you can use combinations too)
        groups[key].add(entity)  # Add entities to their respective groups
    
    # Calculate l-diversity: find the number of distinct entities in each group
    group_diversities = [len(group) for group in groups.values()]
    
    # l-diversity is the smallest number of distinct entities in any group
    if not group_diversities:
        return 1  # No groups found
    
    l_diversity = min(group_diversities)
    return l_diversity

# Function to calculate unicity of the entities
def calculate_unicity(entities):
    """Calculate the proportion of entities that are unique based on quasi-identifiers."""
    qid_combinations = [(entity, entity_type) for entity, context, entity_type in entities]
    counts = Counter(qid_combinations)
    
    # Count how many combinations are unique
    unique_count = sum(1 for count in counts.values() if count == 1)
    total_entities = len(entities)
    
    # Unicity is the proportion of unique records
    unicity = unique_count / total_entities if total_entities > 0 else 0
    return unicity

# # Function to calculate risk score of the entities
def calculate_risk_score(entities):
    """Calculate a risk score based on the uniqueness of quasi-identifiers."""
    qid_combinations = [(entity, entity_type) for entity, context, entity_type in entities]
    counts = Counter(qid_combinations)
    
    # Calculate risk score: the more unique the entity, the higher the risk
    risk_scores = {comb: 1/count for comb, count in counts.items()}
...
# Function to calculate reidentification probability of the entities
def calculate_reidentification_probability(entities):
    """Calculate the probability of re-identification based on entity uniqueness."""
    qid_combinations = [(entity, entity_type) for entity, context, entity_type in entities]
    counts = Counter(qid_combinations)
    
    # Probability of re-identification is inversely related to the frequency of the entity
    reidentification_probabilities = {comb: 1/count for comb, count in counts.items()}
    
    # Average re-identification probability across all entities
    avg_reid_probability = np.mean(list(reidentification_probabilities.values()))
    return avg_reid_probability

# Function to calculate global distribution for t-closeness of the entities
def calculate_global_distribution(entities):
    """Calculate the global distribution of entities in the dataset."""
    # Extract the entity values and types
    entity_values = [(entity, entity_type) for entity, context, entity_type in entities]
    
    # Count the occurrences of each entity type
    counts = Counter(entity_values)
    
    # Normalize the counts to get the global distribution
    total_entities = len(entity_values)
    global_distribution = {entity: count / total_entities for entity, count in counts.items()}
    
    return global_distribution

# Function to calculate t-closeness of the entities
def calculate_t_closeness(entities, global_distribution):
    """Calculate T-closeness by comparing group distributions with the global distribution."""
    groups = defaultdict(list)
    
    # Group entities by quasi-identifiers (entity types in this case)
    for entity, context, entity_type in entities:
        key = (entity_type,)
        groups[key].append(entity)
    
    # Convert global distribution dictionary to a numpy array
    global_distribution_values = np.array(list(global_distribution.values()))
    global_distribution_values = global_distribution_values / global_distribution_values.sum()  # Normalize

    # Calculate T-closeness for each group
    max_t_closeness = 0
    for key, group_entities in groups.items():
        # Calculate group distribution
        group_distribution = Counter(group_entities)
        
        # Ensure group distribution aligns with global distribution
        group_distribution_array = np.array([group_distribution.get(k, 0) for k in global_distribution.keys()])
        group_distribution_array = group_distribution_array / group_distribution_array.sum()  # Normalize
        
        # Compare with global distribution using KL divergence (T-closeness)
        t_closeness = entropy(group_distribution_array, global_distribution_values)
        max_t_closeness = max(max_t_closeness, t_closeness)
