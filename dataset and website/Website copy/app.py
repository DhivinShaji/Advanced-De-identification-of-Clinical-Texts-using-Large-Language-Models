...
import os
from deidentify import calculate_k_anonymity, calculate_unicity, compare_metrics, deidentify_text, deidentify_text1, deidentify_textReplace, extract_entities_with_context, calculate_l_diversity, calculate_t_closeness, calculate_reidentification_probability, calculate_risk_score, calculate_global_distribution, load_clinical_letters_from_xml
...
def process_batch():
    ...
    if action == 'redact':  # If the action is to redact
        ...
        original__risk_score = calculate_risk_score(all_original_entities)
        original_reidentification_probability = calculate_reidentification_probability(all_original_entities)
        original_global_distribution = calculate_global_distribution(all_original_entities)
        original_t_closeness = calculate_t_closeness(all_original_entities, original_global_distribution)
        ...
        replaced__risk_score = calculate_risk_score(all_replaced_entities)
        replaced_reidentification_probability = calculate_reidentification_probability(all_replaced_entities)
        replaced_global_distribution = calculate_global_distribution(all_replaced_entities)
        replaced_t_closeness = calculate_t_closeness(all_replaced_entities, replaced_global_distribution)

        original_metrics = (original_unicity, original_k_anonymity, original_l_diversity, original__risk_score, original_reidentification_probability, original_t_closeness)
        replaced_metrics = (replaced_unicity, replaced_k_anonymity, replaced_l_diversity, replaced__risk_score, replaced_reidentification_probability, replaced_t_closeness)
...
def start_localtunnel(port):
    command = ["lt", "--port", str(port), "--subdomain", "your-subdomain-here"]
